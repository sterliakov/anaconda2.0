...
class MyPy(object):
    ...
    def check_source(self):
        ...
        if '?cwd' in self.settings:
            where = self.settings.index('?cwd')
            del self.settings[where]
            cwd = self.settings[where]
            del self.settings[where]
        else:
        ...
        if '?env' in self.settings:
            where = self.settings.index('?env')
            del self.settings[where]
            f = self.settings[where]
            del self.settings[where]
            with open(f) as env_file:
        ...
        if '?executable' in self.settings:
            where = self.settings.index('?executable')
            del self.settings[where]
            executable = self.settings[where]
            del self.settings[where]
        else:
            executable = sys.executable

        args = shlex.split('{0} -O -m mypy {1} {2} {3} {4} \'{5}\''.format(
            executable, err_sum, err_ctx, dont_follow_imports,
