...
def get_mypy_settings(view):
    ...
    conf = get_settings(view, 'mypy_config_file', None)
    if conf is not None:
        mypy_settings.append('--config-file')
        mypy_settings.append(conf)

    dir_ = get_settings(view, 'mypy_top_directory', None)
    if dir_ is not None:
        mypy_settings.append('?cwd')
        mypy_settings.append(dir_)

    env = get_settings(view, 'mypy_env_file', None)
    if env is not None:
        mypy_settings.append('?env')
        mypy_settings.append(env)

    executable = get_settings(view, 'python_interpreter', None)
    if executable is not None:
        mypy_settings.append('?executable')
        mypy_settings.append(executable)
